...
#import "GPStorageRoomManagedObject.h"
#import "GPAsyncURLConnection.h"
#import "GPCoreDataController.h"

@implementation GPStorageRoomManagedObject
...
+ (void)refreshFromServer:(void(^)(NSArray *currentItems, NSArray *deletedItems))completionBlock checkDeletedItems:(BOOL)checkDeletedItems errorBlock:(void(^)(NSError *error))errorBlock
{
	...
	NSURL *url = [[GPStorageRoom sharedInstance] urlForAllEntriesForClass:[self class]];
	...
	[GPAsyncURLConnection AsyncURLConnectionWithUrl:url userInfo:nil completeBlock:^(NSData *data, id userInfo) {
		...
		NSError *error = nil;
		NSDictionary *jsonInfo = [NSJSONSerialization JSONObjectWithData:data options:0 error:&error];
		if( jsonInfo && !error )
		{
			// Get all objects and iterate
			NSArray *resources = [jsonInfo valueForKeyPath:@"array.resources"];
			NSMutableArray *tmpItems = [NSMutableArray arrayWithCapacity:[resources count]];
			for( NSDictionary *item in resources )
			{
				...
				GPStorageRoomManagedObject *newItem = [self objectWithUniqueIDFromDictionary:item];
			...
			{
				// We *should* check for deleted items: get URL for deleted items
				NSURL *deletedItemsUrl = [[GPStorageRoom sharedInstance] urlForDeletedEntriesForClass:[self class]];

				// Get JSON
				[GPAsyncURLConnection AsyncURLConnectionWithUrl:deletedItemsUrl userInfo:nil completeBlock:^(NSData *data, id userInfo) {

					// Parse JSON
					NSError *error = nil;
					NSDictionary *deletedItemsJsonInfo = [NSJSONSerialization JSONObjectWithData:data options:0 error:&error];
					if( deletedItemsJsonInfo && !error )
					{
						// Extract list of deleted items
						NSArray *deletedResources = [deletedItemsJsonInfo valueForKeyPath:@"array.resources._entry_url"];

						// Fetch from Core Data and delete
						NSFetchRequest *request = [NSFetchRequest fetchRequestWithEntityName:NSStringFromClass( [self class] )];
						request.predicate = [NSPredicate predicateWithFormat:@"%K IN %@", [self uniqueKey], deletedResources];
					...
					// Error getting deleted items. Pass the error to the error block if it exists.
					if( errorBlock )
						errorBlock( error );
				}];
			}
...
+ (NSDictionary *)mappings
{
	 ...
	 * For example:
	 *
	 *		return @{ @"url": ^(NSDictionary *info) {return info[ @"_url" ]; },
	 *				  @"name": ^(NSDictionary *info) {return info[ @"name" ]; }};
	 *
	 * Note: the normal Storageroom meta character @ has been replaced with an underscore (_).
	 */
...
+ (NSString*)uniqueKey
{
	// Use URI as unique key by default. Override to use a different field.
	return @"uri";
}
...
// Utility methods for saving/loading URLs as NSData
